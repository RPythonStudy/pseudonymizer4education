...
- list.files 함수로 파일이름들을 리스트로 가져옵니다. (프로젝트에서는 파일명이 등록번호로 구성되어 있다고 가정합니다.) 
- 파일이름 리스트에서 ::file_path_sans_ext 함수를 이용해서 확장자를 제거한 후, register_numbers라는 문자형 벡터(character vector)로 생성합니다.
